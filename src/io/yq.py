...
from src.utils.df import replace_metric, transpose_df
...
class YahooQueryAPI:
    ...
    def _load_configs(self):
        ...
        metrics = {}
        standard_cols = {}
        for domain in ["income_statement", "cash_flow", "balance_sheet", "relative_valuation"]:
            if domain != "relative_valuation":
    ...
    def get_relative_valuation(self, fa):
        rv = self._pro.add_metrics(fa, self._metrics["relative_valuation"], NEW=True) 
        return rv

    def get_tables(self, fa, rv, domains=["income_statement", "cash_flow", "balance_sheet", "relative_valuation"]):
        dfs = {}
        divisor = 1
        ...
        for symbol in symbols:
            ...
            for domain in domains:
                if domain == "relative_valuation":            
                    if rv is None:
                        rv = self.get_relative_valuation(fa)
                    temp_rv = rv.filter(pl.col("symbol") == symbol)
                    temp_rv = temp_rv.select(self._standard_cols[domain]).drop("symbol")
                    temp_rv = self._format_subsets_df(temp_rv)
                    temp_rv = transpose_df(temp_rv, "date")
                    data[domain] = temp_rv
                else:
                    line_items = self._renames[domain]
                    temp_fa = symbol_fa.select(self._standard_cols[domain]).drop("symbol")
                    temp_fa = temp_fa.rename(line_items)
                    ...
                    temp_fa = self._format_subsets_df(temp_fa)
                    temp_fa = transpose_df(temp_fa, "date")
                    data[domain] = temp_fa
            dfs[symbol] = data
        return dfs

    def _format_subsets_df(self, df):
        ...
        subsets = {
            "Operating Expense": ["SG&A", "R&D", "Others"],
