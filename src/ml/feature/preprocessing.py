...
def features_construction(datasets, transformer, cols, name="transformed"):
...
class CrossSectionManager:
    def __init__(self, target="returns"):
        self.feat = {
            ...
            "num": [],
            "cat": [],
            "target": [target] ##no returns col yet, duplicates of same identifiers for some reason
            }

    def to_cross_section(self, fa, n_dates, n_symbols): 
        symbols = fa["symbol"].unique().to_list()[:n_symbols]
        ...
        self.feat["num"] = sorted(list(set(fa.columns) - set(self.feat["id"]+self.feat["cat"]+self.feat["target"])))
        
        all_dates = fa["date"].unique().sort().to_list()
        valid_dates = fa.drop_nulls(subset=self.feat["target"])["date"].unique().sort().to_list()[-n_dates:]
        ...
        fa = fa.filter(pl.col("date").is_in(dates))
        fa = fa.to_pandas()
