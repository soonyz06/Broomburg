...
class FundamentalManager: 
    ...
    def _load_batch(self, asset_type, frequency, batch_df, exclude_lazy, filedir, partition_map, refresh_threshold_days, REFRESH):
        ...
        source = "yahooquery"
        df, failed_symbols = self._yq.fetch_batch(missing_symbols, self._yq.get_combined_financials, params={"frequency": frequency})
        ...
        if df is None:
            ...
            if df.is_empty():
                ...
                batch_success = df.with_columns(pl.lit(source).alias("source"))
