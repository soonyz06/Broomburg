...
class SECManager:
    ...
    def _fetch_stmts(self, identifiers, filing):
    ...
    def _fetch_section(self, identifiers, filing):
    ...
    def _fetch_text(self, identifiers, filing, filedir):
    ...
    def _fetch_edgar(self, api_key, filings, symbol, func, form, filedir, startDate): 
        print(f"[INFO]Fetching {symbol}: {startDate}") #Types: {filing: metadata, xbrl: statements, obj: everything}
        time.sleep(self._sleep_s)
        ...
        for filing in filings:
            ...
            if func == "statements":
                stmt_df = self._fetch_stmts(identifiers, filing) #1 HTTP request for xbrl
                output = pl.concat([output, stmt_df], how="vertical") if output is not None else stmt_df
            
            if func == "sections":
                section_row = self._fetch_section(identifiers, filing) #1 HTTP request for obj
                if section_row is not None:
                    output.append(section_row)
                
            if func == "texts":
                text_path = self._fetch_text(identifiers, filing, filedir) #1 HTTP request for text
                
        if isinstance(output, list):
            return pl.DataFrame(output)
        return output
    
    def load_sec(self, database, form="10-K", funcs=["statements", "sections", "texts"], refresh_threshold_days=None, REFRESH=False):
        ...
        ##append_parquet auto partitions
        ##check compact job
        ##batch write and tar + zst (symbol_date.txt.gz)?
        #self._writer.append_parquet(filedir, outputs[func], source=source, identifiers=self._identifiers) ##add source
