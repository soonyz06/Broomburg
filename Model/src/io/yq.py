...
class FAProcesser:
    ...
    def _add_history_data(self, financials, period, interval="1d", SAVE=True):
        return 
        #symbols = financials["symbol"].unique(maintain_order=True).to_list()
        #identifiers = ["symbol", "date"]
        #history = get_history(symbols, period, interval) ###fix, mc
...
class YahooQueryAPI:
    ...
    def get_combined_financials(self, symbols, frequency): ##get list of all missing -> fetch async
        ...
        df = df.select(self.standard_cols).sort("symbol", "date") #determinstic schema
        return df
