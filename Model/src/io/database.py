...
class DatabaseManager:
    ...
    def fetch_database(self, asset_type, REFRESH=False, COLLECT=False): 
        ...
        self._writer.append_parquet(filedir, df_final, source="fd", identifiers=self._identifiers)
    ...
    def filter_database(self, asset_type, filters={}, limit=None, REFRESH=False, COLLECT=True):
        ...
        exclude_lazy = self._reader.read_lazy(self._basepath.parent, filename="exclude", identifiers=self._identifiers,
                                              cols=self._identifiers+["asset_type", "error", "source", "timestamp"]) #inner cols: exclude_df
