...
class DatabaseManager:
    ...
    def filter_database(self, asset_type, filters={}, limit=None, REFRESH=False, COLLECT=True):
        ...
        #UNION Excludes
        exclude_filter = {"asset_type": asset_type}
        ...
        exclude_lazy = self._reader.filter_lazy(exclude_lazy, filters=exclude_filter, inclusive=True) 
        if exclude_lazy is None:
            lf = self._limit_lazy(lf, asset_type, limit, COLLECT=COLLECT)
            return lf, None
    ...
    def _limit_lazy(self, lf, asset_type, limit, COLLECT=True):
