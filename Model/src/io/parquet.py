...
class ParquetManager:
    ...
    def _cleanup_backups(self, max_age_hours=24):
        ...
        for f in self._backup_dir.glob("*.parquet"):
            try:
    ...
    def _save_subtable(self, filedir, df, OVERRIDE=False, _BACKUP=False): 
        ...
        if _BACKUP:
            df.write_parquet(self._backup_dir / f"backup_{ts.strftime('%Y%m%dT%H%M%S%fZ')}.parquet") 
    ...
    def _scan_subtable(self, filedir, latest_by_identifiers): 
        filedir.mkdir(parents=True, exist_ok=True) 
        files = list(filedir.glob(f"chunk_*.parquet")) 
    ...
    def read_lazy(self, filedir, latest_by_identifiers): #handles partition pruning from filedir
        filedir.mkdir(parents=True, exist_ok=True) 
        filedirs = list(filedir.rglob(f"chunk_*.parquet")) 
        ...
        for subdir in filedirs:
            ...
            matches = re.findall(r"([^/=]+)=([^/=]+)", str(subdir)) #^: exclusive, +: multiple, []: composite
            lfs.append(lf.with_columns([pl.lit(key).alias(col) for col, key in matches])) #virtual cols
        partition_cols = [col for col, _ in matches] 
        lf = pl.concat(lfs) if lfs else None #partitioning assumes same schema
        return lf, partition_cols

     def save_parquet(self, filedir, df, partition_cols, OVERRIDE=False, _BACKUP=False):     
        if partition_cols is None or not partition_cols:
            self._save_subtable(filedir, df, OVERRIDE=OVERRIDE, _BACKUP=_BACKUP)
            return self
        ...
        for keys, subdf in subtables.items():
            subdir = os.path.join(filedir, *[f"{col}={key}" for col, key in zip(partition_cols, keys)])
            subdf = subdf.drop(partition_cols) #dropped at write and added at read to save memory (virtual col)
            self._save_subtable(subdir, subdf, OVERRIDE=OVERRIDE, _BACKUP=_BACKUP)
        return self
    ...
    def compact_job(self, filedir): #merging and deduplication 
        ...
        self.save_parquet(filedir, lf.collect(), partition_cols, OVERRIDE=True) ##go through everything, holds df in memory, peforms R/W operations in batches to reduce io overhead
        return self
